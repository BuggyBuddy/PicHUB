...
class MyThread(threading.Thread):
	def __init__(self,source,isRecommand):
	...
	def run(self):
		...
		elif self.isRecommand:
			print("not common")
			maxPoss,maxType= Entrance.get_max_dict()
			for i in range(5):
				maxType[i]=maxType[i].replace(' ','%20')
			total = 50
			for i in range(5):
				print(maxType[i], int(maxPoss[i]*total))
				pichub.wpSource.changeSourceWeb("Unsplash")
				pichub.wpSource.changeKeyWord(maxType[i])
				pichub.wpSource.changePerPage(int(maxPoss[i]*total))
				url = self.source.createURL()
				print(url)
				if self.source.crawlerType == "API":
					crawler = APICrawler(self.source.sourceWeb, url)
				...
				self.source.WPList.extend(crawler.imageList) #append
			pichub.showWallPaper()
			pichub.wpSource.changePerPage(9)
...
class WPSource(object):
	...
	def __init__(self, sourceWeb, genre = "temp", keyWord = None):
		...
		self.flag = False
		self.thread = MyThread(self,False)

	def run(self):
		self.thread.terminate()
		self.thread = MyThread(self,pichub.ui.checkBox_Recommend.isChecked())
		self.thread.start()
